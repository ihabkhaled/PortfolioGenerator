...
| `better-auth`                                        | `packages/auth`, `packages/admin-auth` | Auth libraries move fast and touch cookies, sessions and headers. Two owners because `/managawy` runs a second, fully isolated `betterAuth()` instance — see ADR-0010 — not because the boundary is loose. |
...
| `next/navigation`                                    | `packages/navigation`                  | Server-safe half only. The client hooks return in a `client.ts` the day a component needs the router.                                                                                                      |
