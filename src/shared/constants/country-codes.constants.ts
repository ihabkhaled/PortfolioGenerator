...
/**
 * Dialling codes, as a stored list rather than a library.
 *
 * A phone number on a portfolio is a string a reader copies, not something the
 * product parses, formats or validates against a national numbering plan.
 * Pulling in a full libphonenumber build — hundreds of kilobytes of metadata —
 * to render `(+20) 100-156-8256` would be a bad trade for every visitor.
 *
 * The list is deliberately flat: ISO code, display name, dialling prefix. It
 * is sorted by name at the point of use, because sort order is a presentation
 * decision and depends on the reader's locale.
 *
 * Several entries share a prefix (`+1` is the whole North American plan,
 * `+7` covers Russia and Kazakhstan). The ISO code, not the prefix, is the
 * key: two countries can dial the same and still be different answers to
 * "where are you".
 */
export const COUNTRY_DIAL_CODES: readonly CountryDialCode[] = [
  ...
  { iso: 'BV', name: 'Bouvet Island', dial: '+47' },
  ...
  { iso: 'HM', name: 'Heard Island and McDonald Islands', dial: '+672' },
  { iso: 'VA', name: 'Holy See', dial: '+39' },
  ...
  { iso: 'SH', name: 'Saint Helena, Ascension and Tristan da Cunha', dial: '+290' },
  ...
  { iso: 'GS', name: 'South Georgia and the South Sandwich Islands', dial: '+500' },
...
/** Used when a stored number has no country attached to it. */
