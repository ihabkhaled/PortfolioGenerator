...
**Scope:** `src/modules/portfolio-editor/containers/image-crop-field.container.tsx`

## Why the rule fires

The container renders a plain `<img>` instead of `next/image`, which the rule
...
The container also needs a real `<img>` DOM node for two things `next/image`
